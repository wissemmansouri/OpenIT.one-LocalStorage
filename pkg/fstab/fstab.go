...
func (f *FStab) Add(e Entry, replace bool) error {
	...
	if err := copy(f.path, f.path+".openit.bak"); err != nil {
		return err
	}
	...
	_, err = fstabFile.WriteString(e.String() + "\t# Added by the OpenIT.one\n")
	if err != nil {
		return err
	}
...
func (f *FStab) RemoveByMountPoint(mountpoint string, comment bool) error {
	FStabPathNew := f.path + ".openit.new"
