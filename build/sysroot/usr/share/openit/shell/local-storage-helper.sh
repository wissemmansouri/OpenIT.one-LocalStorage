...
#Obtain the insertion path of the disk
#param path /dev/sda
GetPlugInDisk() {
...
#Format FAT32 disk
#param Directory/Partition to be formatted /dev/sda1
#param Format
FormatDisk() {
  if [ "$2" == "fat32" ]; then
    mkfs.vfat -F 32 $1
...
#Remove mount point, delete mounted folder
...
#Add partition, there is only one partition
#param Path /dev/sdb
