...
import (
	...
	"github.com/wissemmansouri/OpenIT.one-LocalStorage/model"
	"github.com/wissemmansouri/OpenIT.one-LocalStorage/pkg/config"
	"github.com/wissemmansouri/OpenIT.one-LocalStorage/pkg/fstab"
	"github.com/wissemmansouri/OpenIT.one-LocalStorage/pkg/mount"
	"github.com/wissemmansouri/OpenIT.one-LocalStorage/pkg/partition"
	"github.com/wissemmansouri/OpenIT.one-LocalStorage/pkg/utils/command"

	"github.com/moby/sys/mountinfo"

	model2 "github.com/wissemmansouri/OpenIT.one-LocalStorage/service/model"
	v2 "github.com/wissemmansouri/OpenIT.one-LocalStorage/service/v2"
	"github.com/wissemmansouri/OpenIT.one-LocalStorage/service/v2/fs"
	"go.uber.org/zap"
	"gorm.io/gorm"
)
	...
	for _, v := range diskList {
		if IsDiskSupported(v) {
			if _, ok := diskMap[v.Serial]; !ok {
				...
				// add UI properties to applicable events so that OpenIT.one UI can render it
...
func (d *diskService) GetSystemDf() (model.DFDiskSpace, error) {
	...
	lines = lines[1:]
	// iterate through each line, parse file information
	for _, line := range lines {
		// split lines, retrieve various fields
